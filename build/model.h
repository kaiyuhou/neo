...
#define PanSource	"/root/neo-kaiyu/src/network.pml"
...
struct comm_state_t { /* user defined type */
	int fib[2];
	unsigned process_id : 1;
	unsigned pkt_state : 4;
	unsigned fwd_mode : 3;
	int ec[2];
	int src_ip[1];
	int seq[1];
...
typedef struct State {
	...
	int comm;
	int num_comms;
	int correlated_policy_idx;
	int choice;
	int choice_count;
	int candidates[2];
	struct comm_state_t comm_state[2];
#ifdef TRIX
